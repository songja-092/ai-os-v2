...
def main():
    p=argparse.ArgumentParser(); p.add_argument("--platform",choices=["youtube","reddit"],required=True); p.add_argument("--seeds",required=True); p.add_argument("--output",required=True); p.add_argument("--memory"); p.add_argument("--limit",type=int,default=10); p.add_argument("--mode",choices=["general","reference_default","marketer_video"],default="general"); a=p.parse_args()
    ...
    collected=[]; failures=[]; source_summary=[]
    for seed in seeds:
        ...
        try:
            ...
            source_summary.append({"channel":seed["channel"],"status":"FAILED","entries":0})
            continue
        source_entries=0
        for entry in root.findall(f"{ATOM}entry"):
            ...
            source_entries+=1
        source_summary.append({"channel":seed["channel"],"status":"CONNECTED","entries":source_entries})
    ...
    if a.mode == "reference_default":
        relevant=[x for x in relevant if "/shorts/" not in (x.get("url") or "")]
    elif a.mode == "marketer_video":
        relevant=[x for x in relevant if "/shorts/" in (x.get("url") or "")]
    ...
    # Prevent one high-frequency channel from hiding every other connected source.
    by_channel={}
    for item in items:
        by_channel.setdefault(item["channel_name"],[]).append(item)
    balanced=[]
    for channel in sorted(by_channel, key=lambda name: by_channel[name][0]["published_at"], reverse=True):
        balanced.append(by_channel[channel].pop(0))
    remainder=sorted((item for channel_items in by_channel.values() for item in channel_items),key=lambda x:x["published_at"],reverse=True)
    balanced.extend(remainder)
    result={"schema_version":"1.1","platform":a.platform,"mode":a.mode,"collected_at":now.isoformat(),"requested_period_days":30,"used_period_days":used,"period_expanded":used>30,"discovery_strategy":"adopted_channel_first_then_temporary_channel","selection_strategy":"one_per_connected_relevant_channel_then_recent","preferred_channels_applied":sorted(preferred_names),"source_summary":source_summary,"items":balanced[:a.limit],"source_failures":failures,"adoption_decision":None}
