...
DEFAULT_CATALOG = PLUGIN_ROOT / "registry" / "candidate-catalog.json"
...
def load_catalog(path: Path) -> list[dict]:
    ...
    if record.get("schema_version") != "1.0" or not isinstance(record.get("candidates"), list):
        raise ValueError("candidate catalog must use schema_version 1.0 and contain candidates[]")
    seen: set[str] = set()
    required = {"id", "repo", "role", "category", "reason", "pm_stage", "current_decision", "install_allowed_now", "activation_gate"}
    for item in record["candidates"]:
        missing = required - set(item)
        if missing:
            raise ValueError(f"candidate catalog item is missing fields: {sorted(missing)}")
        if not ID_RE.fullmatch(item["id"]) or item["id"] in seen:
            raise ValueError(f"invalid or duplicate candidate id: {item['id']}")
        seen.add(item["id"])
...
def cmd_collect(args: argparse.Namespace) -> None:
    ...
    print(json.dumps({
        ...
        "path": str(path),
        "candidates": [
            {k: c.get(k) for k in ("id", "role", "pm_stage", "current_decision", "activation_gate", "stars", "freshness", "license", "url", "actions")}
            for c in candidates
        ],
...
def cmd_review(args: argparse.Namespace) -> None:
    ...
    print(json.dumps({
        ...
        "decision_needed": [
            {k: c.get(k) for k in ("id", "role", "pm_stage", "current_decision", "activation_gate", "reason", "stars", "freshness", "license", "url", "status", "actions")}
