...
- 현재 상태는 `M1_foundation_pass / M2_internal_fixture_allowed / visual_target_approval_pending /
...
- 다음 세션은 기존 확정 답변 12개를 다시 묻지 않고 M2 내부 Fixture를 제작·검사합니다.
  가격·실제 사용자 모집 범위는 해당 사업 마일스톤으로 미루고, 첫 개입은 Visual Target 방향 승인입니다.
...
사용자가 회의 또는 회의 모드를 시작하면 공통 실행계약의 `회의 응답 계약`을 적용해 짧고 쉬운 답변으로 논의합니다. 회의 종료 후에는 작업 과정의 나열보다 합의된 결과·보류 사항·다음 행동을 중심으로 보고합니다.
